...
export default function InstructorPage() {
  ...
  const fetchingData = async () => {
    setLoading(true);
    try {
      const res = await request("instructor", "get");
      if (res) {
    ...
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-2">
            <Users className="w-6 h-6 text-[#003868]" /> Instructors
          </h1>
            ...
            <DialogContent className="max-w-[95vw] md:max-w-2xl">
              ...
              <form onSubmit={onSubmit} className="mt-4">
                ...
                <div className="flex justify-end mt-8 pt-4 border-t border-slate-100 gap-3">
                  ...
                  <Button type="submit" className="px-8 bg-[#003868] hover:bg-[#00284d]">
                    {isEdit ? "Update" : "Save"}
                  </Button>
                </div>
              </form>
      ...
      {loading ? (
        <div className="flex flex-col justify-center items-center py-32 gap-3 text-[#003868]">
          ...
          <span className="font-medium animate-pulse text-sm">Syncing instructors...</span>
        </div>
      ) : instructor?.length === 0 ? (
        <div className="border-2 border-dashed border-slate-200 rounded-xl p-12 flex flex-col items-center justify-center text-slate-400 bg-slate-50/50">
          ...
          <p className="font-medium">No instructors found in the database.</p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:hidden gap-4">
            {instructor.map((item, index) => (
              <div key={item.id || index} className="bg-white border rounded-xl p-5 shadow-sm space-y-4 relative overflow-hidden">
                ...
                <div className="flex flex-col gap-3 text-sm bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <div className="flex items-center gap-3 text-slate-600">
                    ...
                    <span className="truncate font-medium">{item?.email || "N/A"}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600">
              ...
              <TableHeader className="bg-[#003868]">
                <TableRow className="hover:bg-[#003868]">
                  {tbl_head?.map((item, index) => (
                    <TableHead key={index} className="whitespace-nowrap text-white font-bold py-4">
                      {item}
                    </TableHead>
                  ))}
                </TableRow>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="mt-2">
              Are you sure you want to delete <strong className="text-rose-500">{deleteData?.first_name} {deleteData?.last_name}</strong>? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
