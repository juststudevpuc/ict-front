  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        return; // Stop running, don't close the form!
      }
      if (res) {
        fetchingData();
        setIsOpen(false);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-sm w-full sm:w-auto">
              ...
              {query && (
                <X
                  className="w-4 h-4 text-slate-300 cursor-pointer hover:text-rose-500 shrink-0"
                  onClick={() => {
                    fetchingData();
                    setQuery("");
                  }}
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              ...
              <DialogContent className="w-[95vw] sm:max-w-md rounded-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Enter the syllabus and pricing details.
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={onSubmit} className="space-y-4 pt-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      required
                      className="focus-visible:ring-[#003868]"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end">
                    <div className="space-y-2 w-full">
                      ...
                      >
                        <SelectTrigger className="focus-visible:ring-[#003868]">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2 w-full">
                      ...
                      <Input
                        type="file"
                        className="text-[10px] file:text-[#003868] file:font-bold focus-visible:ring-[#003868]"
                        onChange={(e) =>
                  ...
                  <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                    ...
                    <Button
                      type="submit"
                      className="bg-[#003868] hover:bg-[#002b50] px-8"
                    >
        ...
        <Dialog open={isDelete} onOpenChange={setIsDelete}>
          <DialogContent className="w-[90vw] sm:max-w-xs text-center rounded-2xl">
            ...
            <p className="text-sm text-slate-500 mt-2">
              This action cannot be undone for <br />
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:hidden gap-5">
              {course.map((item, index) => (
                <div
                  key={item.id || index}
                  className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm flex flex-col relative group"
                >
                  {/* Status Overlay */}
                  <div className="absolute top-3 right-3 z-10">
                    <div
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full shadow-sm backdrop-blur-md ${item?.status ? "bg-emerald-500/90 text-white" : "bg-slate-800/80 text-white"}`}
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full bg-white ${item?.status ? "animate-pulse" : ""}`}
                      />
                  ...
                  <div className="w-full h-40 bg-slate-100 overflow-hidden relative">
                    <img
                      ...
                      // ✅ FIX: Check for the Cloudinary 'image_url' first, then fallback to 'image'
                      ...
                      onError={(e) => {
                        ...
                        e.target.src =
                          "https://placehold.co/400x200?text=No+Cover+Image";
                      }}
                    />
                  ...
                  <div className="p-4 flex-1 flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2 pt-4 border-t border-slate-100">
                      <Button
                        variant="secondary"
                        className="bg-blue-50 hover:bg-blue-100 text-blue-700 h-9"
                        onClick={() => onEdit(item)}
                      >
                      ...
                      <Button
                        variant="outline"
                        className="border-rose-100 text-rose-600 hover:bg-rose-50 h-9"
              ...
              <Table>
                <TableHeader className="bg-slate-50/80 border-b border-slate-200">
                  <TableRow className="hover:bg-slate-50/80">
                    {tbl_head.map((head, i) => (
                      <TableHead
                        key={i}
                        className="text-[10px] font-black uppercase tracking-widest text-[#003868] py-4"
                      >
                        {head}
                      </TableHead>
                    ))}
                ...
                <TableBody>
                  {course.map((item, index) => (
                    ...
                    >
                      <TableCell className="text-[11px] font-bold text-slate-400 text-center">
                        {index + 1}
                      </TableCell>
                      ...
                      <TableCell>
                        <span className="text-[11px] font-medium text-slate-600 px-2 py-1 bg-slate-100 rounded-md whitespace-nowrap">
                          {item?.description}
                        </span>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div
                          className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg ${item?.status ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-slate-50 text-slate-500 border border-slate-200"}`}
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${item?.status ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`}
                          />
                          <span className="text-[10px] font-black uppercase tracking-wider">
                            {item?.status ? "Active" : "Offline"}
                          </span>
